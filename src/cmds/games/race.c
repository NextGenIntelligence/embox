 ...
 *
 * See main(), move_road_handler() and move_car_handler() functions for
 * the lthread usage example.
 *
...
static int move_road_handler(struct lthread *self) {
	int to_wait;
...
static int move_car_handler(struct lthread* self) {
	goto lthread_resume(self, &&update);
...
int main(int argc, char **argv) {
	...
	lthread_init(&lt_road, move_road_handler);
	lthread_init(&lt_car, move_car_handler);
